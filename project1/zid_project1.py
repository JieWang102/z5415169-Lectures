...
def get_tics(pth):
    ...
    """
    fobj = open(pth, mode='r')
    with open(pth, mode='r') as abcd: #mode r 为读取
        dict = {}
        for line in abcd:
            exchange, tic = list(line.strip().replace('\"', '').lower().split('='))
            dict[tic] = exchange
        return dict
...
def read_dat(tic):
    ...
    pth = os.path.join(DATADIR, f'{tic}_prc.dat')
    fobj = open(pth, mode='r')
    lines = []
    for x in fobj:
...
def line_to_dict(line):
    ...
    for column in COLUMNS:
        dict[column] = line [s : s+COLWIDTHS[column]]
        s += COLWIDTHS[column]
    return dict
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is None:
        ...
        for company in tickers_lst:
            if company in list (tic_exchange_dic.key()):
                continue
            else:
                raise Exception(f"{company} does not correspond to a key tic_exchange_tic.")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is None:
        ...
        for company in col_lst:
            if company in COLUMNS:
                continue
            else:
                raise Exception(f"{company} is not found in 'COLUMNS'")
...
def create_json(data_dict, pth):
    ...
    with open (pth, mode='w') as abcd:
        json.dump(data_dict, abcd)
...
if __name__ == "__main__":
    ...
    _test_create_json(os.path.join(DATADIR, 'data.json'))  # Save the file to data/data.json
    pass